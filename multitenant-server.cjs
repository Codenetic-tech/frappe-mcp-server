 ...
 * Handles multiple Frappe sites from a single process using the frappe-mcp-server library
 */
...
// Import frappe-mcp-server library (must be loaded before starting server)
let mcpLibrary;
...
// Main MCP endpoint - simplified using library
app.post(['/mcp', '/mcp/:siteName'], async (req, res) => {
    // Get site name from URL param, header, or body (in that order of priority)
...
// Start server only after loading library
